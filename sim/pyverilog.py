...
class Buffer(Wire):
    ...
    def __init__(self, width: int, input:Wire, clock:Wire, name: str = None):
        """
        Instantiates a Buffer wire such that all consumers of a wire are synced.

        Args:
            width (int): The width of the wire.
            input (Wire): The wire to the buffer.
...
class Module(ABC):
    ...
    def __init__(self, inputs: dict[str, Wire], outputs: dict[str, Wire], name: str = None):
        """
